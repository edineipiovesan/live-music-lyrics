...
def test_retries_on_503_then_succeeds():
    bad = _resp(503)
    ok = _resp(200)
    with patch("requests.request", side_effect=[bad, ok]), \
...
def test_exhausts_retries_returns_last_response():
    bad = _resp(500)
    with patch("requests.request", return_value=bad), \
         patch("time.sleep"):
...
def test_backoff_doubles_each_attempt():
    bad = _resp(502)
    ok = _resp(200)
    with patch("requests.request", side_effect=[bad, bad, ok]), \
         patch("time.sleep") as mock_sleep:
...
def test_honours_retry_after_header():
    ...
    ok = _resp(200)
    with patch("requests.request", side_effect=[limited, ok]), \
         patch("time.sleep") as mock_sleep:
        result = http_get("http://example.com/", retries=3)
    assert result.status_code == 200
...
def test_429_exhausted_returns_last_response():
    limited = _resp(429)
    with patch("requests.request", return_value=limited), \
         patch("time.sleep"):
...
def test_retries_connection_error_then_succeeds():
    ok = _resp(200)
    with patch("requests.request", side_effect=[requests.ConnectionError("blip"), ok]), \
         patch("time.sleep") as mock_sleep:
        result = http_get("http://example.com/", retries=2, backoff_base=1.0)
    assert result.status_code == 200
    mock_sleep.assert_called_once_with(1.0)
